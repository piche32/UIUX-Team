...
const Title = (props) => {
    return(
        <View style= {{flex: 5, alignItems: 'center', marginVertical: '3%'}}>
          <SvgTitle width = {Dimensions.get('screen').width*0.4}  style={[styles.svgBG, {transform: [{translateX: 22},]} ]}/>
            <Text style={{fontSize: 40, transform: [{translateX: -15},]}}>{props.text}</Text>
          </View>
    );
}
